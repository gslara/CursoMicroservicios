...
public interface AlumnaRepository extends PagingAndSortingRepository<Alumna, Long> {
	...
	@Query("select a from Alumna a where upper(a.nombre) like upper(concat('%', ?1, '%')) or upper(a.apellido) like upper(concat('%', ?1, '%'))") //consulta de Hibernate o JPA Query Language (NO es SQL nativa)
